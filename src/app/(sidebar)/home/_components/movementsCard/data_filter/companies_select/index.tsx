import { useSupplies } from "@/app/(sidebar)/controle/_context";
import { IInstitution } from "@/app/(sidebar)/instituicoes/_context/models/Institution";
import { useInstitutions } from "@/app/(sidebar)/instituicoes/_context/useInstitutions";
import { Button } from "@/components/ui/button";
import {
  Command,
  CommandEmpty,
  CommandGroup,
  CommandInput,
  CommandItem,
  CommandList,
} from "@/components/ui/command";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import { cn } from "@/lib/utils";
import { Check, ChevronsUpDown } from "lucide-react";
import { useState } from "react";

export function CompaniesSelect() {
  const [open, setOpen] = useState(false);

  const { someInstitutions } = useInstitutions();
  const { institution, setInstitution } = useSupplies();

  return (
    <>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            role="combobox"
            aria-expanded={open}
            className="justify-between min-w-[300px]"
          >
            {institution
              ? someInstitutions.find(
                  (institutions) => institutions.email === institution.email
                )?.name
              : "Selecione a instituição"}
            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="min-w-[300px] p-0">
          <Command>
            <CommandInput placeholder="Buscando instituições" />
            <CommandList>
              <CommandEmpty>Nenhuma instituição encontrada.</CommandEmpty>
              <CommandGroup>
                {someInstitutions.map((institutions) => (
                  <CommandItem
                    key={institutions.email}
                    value={institutions.email}
                    onSelect={(currentValue) => {
                      const institution = someInstitutions.find(
                        (institutions) => institutions.email === currentValue
                      );

                      setInstitution(institution ?? null);
                      setOpen(false);
                    }}
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        institution?.email === institutions.email
                          ? "opacity-100"
                          : "opacity-0"
                      )}
                    />
                    {institutions.name}
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
          </Command>
        </PopoverContent>
      </Popover>
    </>
  );
}
